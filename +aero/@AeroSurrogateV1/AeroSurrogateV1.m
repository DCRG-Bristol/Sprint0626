classdef AeroSurrogateV1 < api.AbstractPolar
    ...
    methods
        function obj = AeroSurrogateV1(Taw,opts)
            arguments
                Taw
            ...
            if ~obj.use_original
                low_M_data_files = ["Ascend_total2.mat"];
                for i=1:length(low_M_data_files)
                    idata = load(fullfile(fileparts(mfilename('fullpath')),'private',low_M_data_files(i)));
                    fn = fieldnames(idata);
                    obj.data_ascent = [obj.data_ascent, idata.(fn{1})];
                end
    
                high_M_data_files = ["Cruise_total2.mat"];
                for i=1:length(high_M_data_files)
                    idata = load(fullfile(fileparts(mfilename('fullpath')),'private',high_M_data_files(i)));
                    fn = fieldnames(idata);
                    obj.data_cruise = [obj.data_cruise; idata.(fn{1})];
                end
            end

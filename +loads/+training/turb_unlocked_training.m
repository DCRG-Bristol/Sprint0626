...
for i = 1:nSamples
    ...
    try
        ...
        sizeOpts = util.SizingOpts(IncludeGusts=true, IncludeTurb=false, ...
        ...
        turb_1 = ld.ExtractTurbLoads(fullfile(BinFolder,'bin','sol146.h5'),ld.Tags(2));
        turb_2 = ld.ExtractTurbLoads(fullfile(BinFolder,'bin','sol146.h5'),ld.Tags(3));
        turb_my_1 = max(abs(turb_1(1).My), [], 1);  % Inner span bending moment
        turb_my_2 = max(abs(turb_2(1).My), [], 1);  % Outer span bending moment
        ...
        if abs(hh_1(end) - hh_2(1)) < 1e-6
            hh_2 = hh_2(2:end);
            turb_my_2 = turb_my_2(2:end);
        end
        
        hh = [hh_1, hh_2];
        spanNorm = hh./max(hh);
        
        turb_full = [turb_my_1, turb_my_2];               % Concatenate along the span-wise direction
        
        % Ensure spanNorm is strictly increasing and unique
        [spanNorm, idx] = unique(spanNorm, 'stable');
        turb_full = turb_full(idx);

        % Interpolate moment to common station grid
        myInterp = interp1(spanNorm, turb_full, stationGridNorm, 'pchip', 'extrap');

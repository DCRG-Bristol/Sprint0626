function Y = physical_model_indep_sweep_ar_he_uncertain_fuel_price(X, det_P)
%% Title section - Total operating cost (TOC) computations for the swept wing case (uncertain fuel price)
%{
--------------------------------------------------------
Comments:
* The model computes the total operating cost as a function of design variables (i.e., Sweep angle, Mach no.) and uncertain variables (i.e., Fuel price, Oil price)
* X: vector of uncertain variables; det_P: vector of design variables; Y: vector of model outputs
...
* Y: vector of quantities of interest (QIs)
    * Y(1)      : total operating cost (TOC)
--------------------------------------------------------
%}

%%
fuel_price = X(1); % USD/kg
oil_price = X(2); % USD/kg
...
ADP.AR = det_P(3);
ADP.HingeEta = det_P(4);
ADP.FlareAngle = 15;
ADP.ADR.M_c = det_P(2);
ADP.SweepAngle = det_P(1);
...
cd0 = ADP.AeroSurrogate.Get_Cd(0,det_P(2),FlightPhase.Cruise);
cd_cruise = ADP.AeroSurrogate.Get_Cd(Cl_cruise,det_P(2),FlightPhase.Cruise);
% collate data
Y = C_ops;

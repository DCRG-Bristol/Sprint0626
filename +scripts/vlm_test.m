...
vlm_model = laca.vlm.Model.From_laca_model(model,10,1,true);
vlm_model.Stitch();
f = figure(3);clf;
vlm_model.draw
f.CurrentAxes.ZDir = 'Reverse';
...
vlm_simple.Normalwash = vlm_simple.Normalwash.*0;
...
% subplot(2,1,1)
vlm_simple.apply_result_katz(1.225);
vlm_simple.draw('param','Cl');
vlm_simple.draw_rings();
ax = gca;
ax.Clipping = 'off';
axis equal

vlm_simple.L(:,1:24)
L = vlm_model.Get_Prop('L'); L(1:24)'

f = figure(10);clf;
% subplot(2,1,1)
idx = vlm_simple.WingIDs([1,2]);
idx = [idx{:}];
plot(vlm_simple.Collocation(2,idx),vlm_simple.Lprime(idx))

%% trim simple
weight = (m*9.81);

tic;
tt = fminsearch(@(aoa)(trim_sol_simple(vlm_simple,aoa,0,CoM)-weight)^2-weight,1)
toc;

f = figure(8);clf;
vlm_simple = vlm_simple.apply_result_katz(1.225);
...
[L,p] = trim_sol_simple(vlm_model,X(1),X(2),CoM);
act = [L;p];
target = [weight;0];
val = norm(act-target);
end
